...
const EnlargedImage = ({ url, setEnlarge }) => {
  return (
    <>
      <div style={{ display: 'flex', justifyContent: 'space-around', alignItems: 'center', position: 'absolute', left: '0', top: '0', height: '100vh', width: '100vw', backgroundColor: 'grey', opacity: '0.8', overflow: 'hidden' }} onClick={() => {
        document.body.style.overflow = 'inherit'
        setEnlarge({ enlarge: false, url: ''})
      }}>
        <img src={url} alt={url} style={{ position: 'relative', height: '75vh', width: '75vw' }} onClick={(e) => { e.stopPropagation() }}/>
      </div>
    </>
  )
}
  ...
  return (
    <>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'space-between', margin: '2rem auto', width: '80%' }}>
        ...
        { enlarge.enlarge && <EnlargedImage url={enlarge.url} setEnlarge={setEnlarge}/> }
      </div>
    </>
  )
}
